 ...
 * Extension v0.4.15 TEST
 *
 * CAMBIOS v0.4.15
 * - Corrige information_schema.columns:
 *   utiliza data_type en lugar de column_type.
 *
 * - Corrige inferencia de fechas:
 *   los valores numéricos NO se consideran fechas
 *   únicamente por estar dentro del rango de fechas
 *   seriales de Excel.
 *
 * - Fecha se detecta como DATE principalmente por
 *   encabezado o por valores de fecha reales.
 *
 * - Año, Local, Transshipment y Total TEUs
 *   se mantienen como BIGINT cuando corresponde.
 *
 * - Detecta variantes:
 *   Total TEUs
 *   Total TEU's
 ...
 *   Total
 *
 * - Resultados Arrow/DuckDB-Wasm normalizados.
 *
...
(() => {
  ...
  const APP_ID = "tm-excel-engine-v0415-test";
  const VERSION = "v0.4.15-test";
  ...
  /*
   * Normaliza nombres de columnas solamente
  ...
  /*
   * Convierte valores Arrow/DuckDB-Wasm que
   * puedan venir representados como:
   *
   * [1535012, 0, 0, 0]
   *
   * en:
   ...
   * IMPORTANTE:
   *
   * Aquí NO utilizamos el rango 20000-80000
   * para declarar que un número es fecha.
   *
   * Eso provocaba que valores como:
   *
   * Local = 87840
   * Transshipment = 31391
   *
   * fueran interpretados erróneamente.
   */
  ...
  function inferColumnType(
    ...
    /*
     * Primero respetamos encabezados conocidos.
     */
    ...
    /*
     * Fecha solamente por valores que realmente
     * parezcan fecha, pero nunca por un número.
     */
  ...
  function normalizeExcelDate(
    ...
    /*
     * Solamente convertimos serial Excel
     * cuando la columna ya fue identificada
     * como DATE.
     */
    if (
      typeof value === "number" &&
   ...
   * CORRECCIÓN v0.4.15:
   *
   * DuckDB-Wasm utilizado aquí expone
   * data_type, no column_type.
   */
...
ORDER BY "Año";</textarea>
   ...
   */
  if (
    window.__TM_EXCEL_ENGINE_0415_INITIALIZED
  ) {
    return;
  }
