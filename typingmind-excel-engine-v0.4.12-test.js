 ...
 * Extension v0.4.12 TEST
 *
 * CAMBIOS v0.4.12
 * - Detecta "Total TEUs" y variantes como "Total TEU's".
 * - El botón "Total por año" usa los nombres REALES de las columnas.
 * - Normaliza resultados Arrow/DuckDB-Wasm, incluyendo valores agregados
 *   representados como arreglos de 4 palabras.
 * - Mantiene procesamiento 100% LOCAL.
 * - DuckDB-Wasm 1.29.0 / DuckDB interno 1.1.1
 * - SheetJS 0.18.5
 */
(() => {
  "use strict";

  const APP_ID = "tm-excel-engine-v0412-test";
  const VERSION = "v0.4.12-test";
  ...
  /*
   * Normaliza nombres de columnas solamente para comparaciones.
  ...
  function normalizeNumericArray(value) {
    ...
    /*
     * DuckDB-Wasm puede devolver algunos agregados como
     * [valor,0,0,0].
     */
  ...
  function normalizeValue(value, key = "") {
    ...
    if (ArrayBuffer.isView(value)) {
      ...
      return arr.map(v => normalizeValue(v, key));
    }
  ...
  function setResult(value) {
    const safe = jsonSafe(normalizeValue(value));

    const output = document.getElementById("tm-excel-result");
  ...
  function makeUniqueHeaders(headers) {
    ...
    headers.forEach((header, index) => {
      let name = safeString(header).trim();
      ...
      const count = used.get(normalized) + 1;
  ...
  function detectSpecialColumns() {
    ...
    return {
      ...
      local: localColumn,
      transshipment: transshipmentColumn
    };
  }
  ...
  function inferColumnType(
    ...
    if (
      ...
      values.filter(looksLikeDate).length ===
        values.length
    ) {
      return {
        column_index: columnIndex,
    ...
    if (
      values.filter(looksLikeInteger).length ===
      values.length
    ) {
      return {
        column_index: columnIndex,
    ...
    if (numericCount === values.length) {
      return {
        ...
        sqlType: "DOUBLE",
        confidence: "medium",
        reason: "todos los valores son numéricos",
        non_empty_values
      };
    }
    ...
    return {
      ...
      sqlType: "VARCHAR",
      confidence: "medium",
      reason: "valores tratados como texto",
      non_empty_values
    };
  }
  ...
  function buildCreateTableSql(
    ...
    const columns = headers.map(
      (header, index) => {
        const type =
          inferredTypes[index]?.sqlType ||
          "VARCHAR";

        return (
          `${escapeSqlIdentifier(header)} ` +
          type
        );
      }
    );
  ...
  async function totalByYear() {
    try {
      ...
      /*
       * IMPORTANTE:
       * Ya no buscamos literalmente "Total TEU's".
       * Utilizamos el nombre real encontrado
       * en el encabezado del Excel.
       */
      if (!yearColumn) {
        throw new Error(
...
ORDER BY "Año";</textarea>
    ...
    /*
     * Intentamos integrarlo como panel lateral de TypingMind.
     * Si el contenedor específico no existe,
     * se utiliza un panel flotante.
     */
    const possibleHosts = [
      "#right-sidebar",
    ...
    for (const selector of possibleHosts) {
      host =
  ...
  async function initialize() {
    try {
      ...
      /*
       * Guardamos información útil en window
       * para diagnóstico desde la consola.
       */
   ...
   * si TypingMind vuelve a evaluar el script.
   */
  if (
    window.__TM_EXCEL_ENGINE_0412_INITIALIZED
  ) {
    return;
  }
