 ...
 * Extension v0.4.18 TEST
 *
 * CAMBIOS v0.4.18
 *
 * - Basado completamente en v0.4.17.
 *
 * - Mantiene la corrección de enteros negativos
 *   provenientes de Arrow / TypedArray.
 *
 * - MEJORA PRINCIPAL:
 *   Utiliza row.toJSON() cuando está disponible
 *   para convertir correctamente los resultados
 *   Arrow antes de la normalización.
 *
 * - Esto evita depender exclusivamente de patrones
 *   de TypedArray para interpretar resultados.
 *
...
(() => {
   ...
   * ============================================================
   *
   * Convierte correctamente UInt32 representando
   * enteros Int32 negativos.
   *
   * Ejemplo:
   *
   * 4294967295 -> -1
  ...
  /*
   * Detecta el patrón de array observado anteriormente
   * en resultados Arrow / TypedArray.
   */
  ...
  /*
   * Normalización general de valores.
   */
  function normalizeValue(
    ...
    if (
      ...
      if (
        scalar !== null &&
        /sum|avg|min|max|count|total|registros|diferencia|variacion|acumulado/i.test(
          key
        )
      ) {
        return scalar;
   ...
   * ============================================================
   *
   * Convierte una fila Arrow utilizando toJSON()
   * cuando DuckDB lo proporciona.
   *
   * DuckDB-Wasm recomienda row.toJSON()
   * para convertir resultados Arrow a JSON.
   */
  function normalizeArrowRow(row) {
    try {
      ...
      // Continuar con el objeto original.
    }
   ...
   * ============================================================
   *
   * Primero intenta utilizar row.toJSON().
   * Si no existe, conserva el comportamiento anterior.
   */
...
ORDER BY "Año";</textarea>
   ...
   */
  if (
    window.__TM_EXCEL_ENGINE_0418_INITIALIZED
  ) {
    return;
  }
