 ...
 * Extension v0.4.17 TEST
 *
 * CAMBIOS v0.4.17
 *
 * - Mantiene todas las correcciones de v0.4.16.
 *
 * - CORRECCIÓN PRINCIPAL:
 *   Corrige resultados numéricos negativos provenientes
 *   de Arrow / TypedArray.
 *
 * - Evita interpretar valores como:
 *
 *       [4294965331, 4294967295, 4294967295, 4294967295]
 *
 *   como un número positivo.
 *
 * - Convierte correctamente representaciones
 *   UInt32 de dos complementos a Int32 cuando
 *   corresponde.
 *
 * - Ejemplo:
 *
...
(() => {
  ...
  /*
   * Normaliza nombres de columnas únicamente
   ...
   * ============================================================
   *
   * Convierte correctamente un entero UInt32
   * que representa un entero Int32 negativo.
   *
   * Ejemplo:
   *
   * 4294967295 -> -1
   * 4294965331 -> -1965
   *
   * Solamente se aplica al rango UInt32 superior
   * donde existe una representación negativa.
   */
  ...
  /*
   * Determina si un array de cuatro posiciones
   * parece representar un valor escalar codificado
   * por Arrow / TypedArray.
   *
   * Ejemplo defectuoso observado:
   *
   * [
   *   4294965331,
   *   4294967295,
   ...
   * ]
   *
   * El primer elemento contiene el valor.
   */
  function normalizeNumericArray(value) {
    ...
    /*
     * Aplicar corrección signed 32-bit
     * únicamente al patrón observado.
     */
    const signed =
      nums.map(
    ...
    /*
     * Si los últimos tres valores son cero,
     * el primero representa el escalar.
     */
    if (
      signed[1] === 0 &&
      signed[2] === 0 &&
    ...
    /*
     * Patrón observado en los resultados
     * negativos de DuckDB/Arrow.
     *
     * Ejemplo:
     *
     * [
     ...
     * ]
     *
     * El primer valor es el resultado real.
     */
    if (
      signed[1] === -1 &&
      signed[2] === -1 &&
  ...
  function normalizeValue(
    ...
    if (
      ...
      if (
        scalar !== null &&
        /sum|avg|min|max|count|total|registros|diferencia|variacion/i.test(
          key
        )
      ) {
        return scalar;
      ...
      /*
       * Si es TypedArray numérico normal,
       * convertir sus elementos respetando
       * signed/unsigned.
       */
      return arr.map(
        (v) =>
...
ORDER BY "Año";</textarea>
   ...
   */
  if (
    window.__TM_EXCEL_ENGINE_0417_INITIALIZED
  ) {
    return;
  }
